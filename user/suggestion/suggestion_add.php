...
if ($account_flag_arr[0]["user_flag"] != $flag) { //アカウントの権限とページの権限が一致しない場合
  $_SESSION['TeamA']['error_message'] = "suggestion_add-アクセスする権限がありません";   //アクセス権限が無い場合セッションにエラーメッセージを追加
...
if (!empty($_POST["title"])) { //タイトルが入力されている場合
  $suggestion_obj->insert_e_suggestionbox($_SESSION['TeamA']['account_id'], $_POST["title"], $_POST["textarea1"], $_FILES["input_file"]);  //記事を追加
  header("location: suggestion.php"); //アカウント管理トップページへリダイレクト
  exit();
}
          ...
          <div class="col-md-12">
            <div class="card">
              <div class="card-header">
                <h4 class="card-title">目安箱新規追加</h4>
                <form action="" method="post" name="suggestion_add_form" class="suggestion_add_form" enctype="multipart/form-data">
